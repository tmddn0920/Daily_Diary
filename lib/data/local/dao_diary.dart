...
class DiaryDao extends DatabaseAccessor<AppDatabase> with _$DiaryDaoMixin {
  ...
  Future<Diary?> getDiaryByDate(DateTime date) {
    return (select(diaries)..where((tbl) => tbl.date.equals(date))).getSingleOrNull();
  }

  Future<int> insertDiary(DiariesCompanion diary) => into(diaries).insert(diary);

  Future<bool> updateDiary(Diary diary) => update(diaries).replace(diary);

  Future<int> deleteDiary(int id) => (delete(diaries)..where((tbl) => tbl.id.equals(id))).go();
}
